...
def make_dataset(max_photo, outfile):
    ...
    glob_images("./keio7000-ok", 0,max_photo)
    glob_images("./keio8000-ok", 1,max_photo)
    glob_images("./keio9000-ok", 2, max_photo)
    glob_images("./keio5000-ok", 3, max_photo)
    x = np.array(x, dtype=np.float32)
    np.savez(outfile,x=x,y=y)
    print("saved:" + outfile)
    
make_dataset(4, "keio-photo-min.npz")
